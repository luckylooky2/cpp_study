...
// 1. 기본적으로 포인터는 주소를 가리키지만 다음과 같이 널을 가리킬 수도 있다. => 유효하지 않는 공간을 가리키게 되면 당연히 에러를 발생시키며 프로그램에 문제를 줄 수 있다.
// 2. 그런데 포인터가 강력한 이유는 전달 받은 주소에 덧셈뺄셈하면서 다음과 같이 다른 주소에도 자유롭게 접근이 가능하다는 것이다.
// 이 말은 반대로 생각하면 포인터를 통해 할당되지 않은 메모리 공간 또는 다른 용도로 사용되고 있는 메모리 공간에 임의로 접근할 수 있다는 의미이다.
...
int	main()
{
	...
	// // error: cannot assign to variable 'ptr_i' with const-qualified type 'int *const'
	ptr_i = &length;
	// // error: cannot assign to variable 'ref_i' with const-qualified type 'const int &'
	ref_i = length;
